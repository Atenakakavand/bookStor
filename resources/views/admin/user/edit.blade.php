...
@section('content')
    <div class="middle"><!-- start middle -->
        <h1 class="text-center my-4 title-box">ویرایش اطلاعات کاربر</h1>

        <form action="{{route('user.update',['id'=>$users->id]) }}" method="post" class="text-right mr-5 mb-3" enctype="multipart/form-data">
            {{csrf_field()}}
            {{ method_field('PATCH') }}
            ...
            <div class="form-group mt-3">
                ...
                <input type="text" class="form-control ml-2" id="name" name="name"
                       value="{{$users->name}}" placeholder="نام کاربری را وارد کنید">
            </div>
            <div class="form-group mt-3">
                <label>ایمیل کاربر</label>
                <input type="email" class="form-control ml-2" id="email" name="email"
                       value="{{$users->email}}" placeholder="ایمیل کاربر را وارد کنید">
            </div>
            <div class="form-group mt-3">
                <label> نقش کاربر</label>
                <select name="role" id="role" value="{{$users->role}}" class="form-control border">
